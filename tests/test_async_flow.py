...
from nexus_core.structs import CVAnalysis, JDAnalysis, GapAnalysis, Question, AnswerScoreLLM, ScoreDetail, RubricScores
...
class TestAsyncFlow(unittest.TestCase):
    ...
    def test_interview_flow(self, mock_llm):
        ...
        scores_obj = RubricScores(
            role_alignment=ScoreDetail(score=8, evidence="E", reasoning="R"),
            technical_depth=ScoreDetail(score=9, evidence="E", reasoning="R"),
            evidence_of_impact=ScoreDetail(score=8, evidence="E", reasoning="R"),
            communication_clarity=ScoreDetail(score=9, evidence="E", reasoning="R"),
            problem_solving=ScoreDetail(score=8, evidence="E", reasoning="R"),
            star_compliance=ScoreDetail(score=8, evidence="E", reasoning="R"),
            soft_skills=ScoreDetail(score=9, evidence="E", reasoning="R"),
            adaptability=ScoreDetail(score=8, evidence="E", reasoning="R"),
            culture_fit=ScoreDetail(score=9, evidence="E", reasoning="R"),
            professionalism=ScoreDetail(score=9, evidence="E", reasoning="R"),
        )

        mock_score = AnswerScoreLLM(
            scores=scores_obj,
            average_score=8.5
        )
        ...
        async def run_test():
            ...
            self.assertEqual(session.scores[0].average_score, 8.5)
